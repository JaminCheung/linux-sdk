...
static int proc_parse_start(struct proc_parse_info *pi)
{
	...
	if (ret == -1) {
		LOGE("cannot read \"%s\"", MTD_PROC_FILE);
		goto out_free;
	}

	if (ret < PROC_MTD_FIRST_LEN ||
	    memcmp(pi->buf, PROC_MTD_FIRST, PROC_MTD_FIRST_LEN)) {
	    LOGE("\"%s\" does not start with \"%s\"", MTD_PROC_FILE,
		       PROC_MTD_FIRST);
		goto out_free;
	}
...
static int proc_parse_next(struct proc_parse_info *pi)
{
	...
	ret = sscanf(pi->next, PROC_MTD_PATT, &pi->mtd_num, &pi->size,
		     &pi->eb_size);
	if (ret != 3) {
	    LOGE("\"%s\" pattern not found", PROC_MTD_PATT);
	    return -1;
	}

	p = memchr(pi->next, '\"', pi->data_size - pos);
	if (!p) {
	    LOGE("opening \" not found");
	    return -1;
	}
	p += 1;
	pos = p - pi->buf;
	...
	len = p1 - p;
	if (len > MTD_NAME_MAX) {
		LOGE("too long mtd%d device name", pi->mtd_num);
		return -1;
	}
...
int legacy_get_dev_info(const char *node, struct mtd_dev_info *mtd)
{
	...
	if (stat(node, &st)) {
		LOGE("cannot open \"%s\"", node);
		if (errno == ENOENT)
			LOGW("MTD subsystem is old and does not support "
	...
	if (mtd->major != MTD_DEV_MAJOR) {
		errno = EINVAL;
		LOGE("\"%s\" has major number %d, MTD devices have "
	...
	if (ioctl(fd, MEMGETINFO, &ui)) {
		LOGE("MEMGETINFO ioctl request failed");
		goto out_close;
	}
	...
	if (ret == -1) {
		if (errno != EOPNOTSUPP) {
			LOGE("MEMGETBADBLOCK ioctl failed");
			goto out_close;
		}
		errno = 0;
		mtd->bb_allowed = 0;
	...
	switch(mtd->type) {
	case MTD_ABSENT:
	    LOGE("mtd%d (%s) is removable and is not present",
		       mtd->mtd_num, node);
		goto out_close;
	case MTD_RAM:
	...
	LOGE("mtd%d not found in \"%s\"", mtd->mtd_num, MTD_PROC_FILE);
	errno = ENOENT;
	return -1;
